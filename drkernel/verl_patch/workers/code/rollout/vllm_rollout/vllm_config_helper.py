...
Configured for vLLM 0.18.0+ where processed_logprobs includes ALL transformations
...
def get_vllm_config_kwargs(config):
    ...
    if use_v1:
        ...
        # In vLLM 0.18.0+, processed_logprobs includes ALL transformations (penalties, temperature, top-k/top-p)
        ...
        print("Using V1 sampler with processed_logprobs mode (vLLM 0.18.0+)")
        ...
        print("WARNING: V0 sampler is deprecated and removed in modern vLLM releases")
        print("Please set VLLM_USE_V1=1 to use the V1 engine")
        raise ValueError("V0 engine is not supported in modern vLLM releases. " "Please set VLLM_USE_V1=1 to use the V1 engine.")
...
def verify_importance_sampling_safety(config):
    ...
    With vLLM 0.18.0+, all configurations are safe with V1 sampler + processed_logprobs.
    ...
    # With vLLM 0.18.0+, V1 sampler with processed_logprobs is always safe
    print("Using V1 sampler with processed_logprobs (vLLM 0.18.0+)")
    ...
    if min_p > 0:
        print(f"Note: min_p={min_p} is active (filters tokens below {min_p*100:.1f}% of top token's probability)")

    if temperature == 0:
        print("Note: temperature=0 means greedy sampling (deterministic)")
    elif temperature < 0.5:
        print(f"Note: low temperature ({temperature}) makes sampling more focused")
    elif temperature > 1.5:
        print(f"Note: high temperature ({temperature}) makes sampling more diverse")
